...
export default function LarcPortalLogin() {
  ...
  async function submit(e) {
    ...
    setErr(''); setBusy(true)
    try {
      const { data } = await larcPortalApi.post('/login', { dob, phone_last4: last4 })
      nav('/larc-portal/verify', { state: { challenge_token: data.challenge_token } })
    } catch (e) {
      const detail = e?.response?.data?.detail
      setErr(typeof detail === 'string' ? detail : 'Sign-in failed.')
