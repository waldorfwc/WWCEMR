...
export default function PelletManual() {
  const qc = useQueryClient()
  const { has } = useCurrentUser()
  const canEdit = has?.('pellet:manage')
  const [editingId, setEditingId] = useState(null)
  const [adding, setAdding] = useState(false)

  const { data: sections = [], isLoading } = useQuery({
    queryKey: ['pellet-manual'],
    queryFn: () => api.get('/pellets/manual').then(r => r.data),
  })
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-baseline justify-between mb-3 flex-wrap gap-2">
        <div className="flex items-center gap-3">
          <Link to="/pellets/inventory" className="text-muted hover:text-plum-700">
            <ArrowLeft size={18} />
          </Link>
          <div>
            <h1 className="font-serif text-[24px] font-semibold text-ink m-0 flex items-center gap-2">
              ...
              Pellet Operating Manual
            </h1>
            <div className="text-muted text-[12px] mt-0.5">
              Working rules for the WWC pellet inventory + DEA Schedule III workflow.
      ...
      {sections.map(s => (
        <Section
          ...
          onSaved={() => { setEditingId(null); qc.invalidateQueries({ queryKey: ['pellet-manual'] }) }}
          onDeleted={() => qc.invalidateQueries({ queryKey: ['pellet-manual'] })}
        />
      ))}
      ...
      {adding && (
        <AddSectionForm
          ...
          onSaved={() => { setAdding(false); qc.invalidateQueries({ queryKey: ['pellet-manual'] }) }}
        />
      )}
...
function Section({ section, canEdit, editing, onStartEdit, onCancel, onSaved, onDeleted }) {
  ...
  const save = useMutation({
    mutationFn: () => api.patch(`/pellets/manual/${section.id}`, {
  ...
  const remove = useMutation({
    mutationFn: () => api.delete(`/pellets/manual/${section.id}`),
    onSuccess: onDeleted,
...
function AddSectionForm({ onClose, onSaved }) {
  ...
  const create = useMutation({
    mutationFn: () => api.post('/pellets/manual', {
