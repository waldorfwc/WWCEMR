...
def _render(body: str, surgery: Surgery) -> str:
    ...
    # Build "Month D, YYYY" without %-d / %#d — the GNU/BSD extension
    # crashes on Windows interpreters. patient_sms.py already fixed this
    # exact pattern; this file was missed. (Fable recalls audit M7.)
    date_str = (sd.strftime("%B ") + str(sd.day) + sd.strftime(", %Y")
                  if sd else "")
    return (body
...
def create_template(payload: TemplatePayload,
                       ...
                       user: dict = Depends(requires_tier(Module.SURGERY, Tier.MANAGE))):
